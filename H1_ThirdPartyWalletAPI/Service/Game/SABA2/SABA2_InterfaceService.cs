...
{
    public interface ISaba2InterfaceService : IGameInterfaceService
    {
        public Task<int> PostSabaRecord(SABA_Game_Record recordData);
    ...
    public class SABA2_InterfaceService : ISaba2InterfaceService
    {
        private readonly ILogger<SABA2_InterfaceService> _logger;
        ...
        public SABA2_InterfaceService(ILogger<SABA2_InterfaceService> logger,
        ...
        public async Task<MemberBalance> GetGameCredit(Platform platform, GamePlatformUser platform_user)
        {
            ...
            SABA_CheckUserBalance_Res getBalanceResult = await _gameApiService._Saba2API.CheckUserBalance(getBalanceData);
            ...
            else
            {
                ...
                _logger.LogError("Saba2餘額取得失敗 Code:{errorCode} Msg: {Message}", getBalanceResult.error_code, getBalanceResult.message);
            }
            Balance.Wallet = nameof(Platform.SABA2);
            return Balance;
        }
        public async Task<bool> KickUser(Platform platform, GamePlatformUser platform_user)
        {
            ...
            SABA_KickUser_Res resData = await _gameApiService._Saba2API.KickUser(reqData);
            if (resData.error_code != (int)SABA_KickUser_Res.ErrorCode.Success)
            {
                _logger.LogInformation("踢出SABA2使用者失敗 id:{account} Msg: {Message}", platform_user.game_user_id, resData.message);
            }
            return true;
        }
        ...
        public async Task<string> Deposit(GamePlatformUser platform_user, Wallet walletData, WalletTransferRecord RecordData)
        {
            try
            {
                ...
                SABA_FundTransfer_Res transferResult = await _gameApiService._Saba2API.FundTransfer(transferData);
                ...
                {
                    RecordData.status = nameof(WalletTransferRecord.TransferStatus.fail);
                    _logger.LogError("FundTransferInSaba2Fail Msg: {Message}", transferResult.message);
                }
            }
            catch
            {
        ...
        public async Task<GamePlatformUser> CreateGameUser(ForwardGameReq request, Wallet userData)
        {
            ...
            requestData.username = Config.CompanyToken.SABA2_Operatorid + "_" + Config.OneWalletAPI.Prefix_Key + userData.Club_Ename;
            requestData.vendor_member_id = Config.CompanyToken.SABA2_Operatorid + "_" + Config.OneWalletAPI.Prefix_Key + userData.Club_id;

            SABA_ResBase result = await _gameApiService._Saba2API.CreateMember(requestData);
        ...
        public async Task<string> Login(ForwardGameReq request, Wallet userData, GamePlatformUser platformUser)
        {
            ...
            SABA_GetSabaUrl_Res urlResult = await _gameApiService._Saba2API.GetSabaUrl(urlData);
        ...
        public async Task<CheckTransferRecordResponse> CheckTransferRecord(WalletTransferRecord transfer_record)
        {
            ...
            reqData.vendor_trans_id = Config.CompanyToken.SABA2_Operatorid + "_" + transfer_record.id.ToString();
            SABA_CheckFundTransfer_Res reuslt = await _gameApiService._Saba2API.CheckFundTransfer(reqData);
            ...
            {
                if (transfer_record.target == nameof(Platform.SABA2))//轉入SABA直接改訂單狀態為失敗
                {
                ...
                else if (transfer_record.source == nameof(Platform.SABA2))
                {
            ...
            {
                if (transfer_record.target == nameof(Platform.SABA2))//轉入SABA直接改訂單狀態為成功
                {
                    CheckTransferRecordResponse.LockCreditChange = -transfer_record.amount;
        ...
        public async Task<dynamic> GetBetRecords(GetBetRecordReq RecordReq)
        {
           ...
           var saba_results = new List<SABA_BetDetails>();
            ...
            {
                var results = await _sabaDbService.GetSabaRecordV2BySummary(createTime, RecordReq.ReportTime, Config.CompanyToken.SABA2_Operatorid + "_" + Config.OneWalletAPI.Prefix_Key + RecordReq.ClubId);
                results = results.OrderByDescending(e => e.partition_time).ToList();
                foreach (var result in results)
                {
                    saba_results.Add(result);
                }
            }
            if(!saba_results.Any())
                saba_results = await _sabaDbService.GetSabaRecordBySummary(RecordReq);

            await SabaRecordPaser(saba_results);
            res.Data = saba_results.OrderByDescending(e => e.transaction_time).Cast<dynamic>().ToList(); ;
            return res;
        }
        public async Task<string> GameDetailURL(GetBetDetailReq RecordDetailReq)
        {
            ...
            SABA_GetBetDetailByTransID_Res resData = await _gameApiService._Saba2API.GetBetDetailByTransID(reqData);
        ...
        public async Task<string> RepairGameRecord(RepairBetSummaryReq RepairReq)
        {
            ...
            if (RepairReq.SearchType == 1)
            {
                ...
                while (df.TotalSeconds > 3600)
                {
                    ...
                    _logger.LogDebug("Rub Repair Saba2 Record StartTime : {start} EndTime : {end}", RepairReq.StartTime, RepairReq.EndTime);
            ...
            {
                while (RepairReq.StartTime <= RepairReq.EndTime)
                {
                    RepairCount += await RepairSaba(RepairReq);
                    RepairReq.StartTime = RepairReq.StartTime.AddDays(1);
                }
            }
        ...
        public async Task<ResCodeBase> SetLimit(SetLimitReq request, GamePlatformUser gameUser, Wallet memberWalletData)
        {
            ...
            var betSettings = System.Text.Json.JsonSerializer.Deserialize<List<SABA_BetSetting>>(requestData.bet_setting);
            //if(betSettings.FirstOrDefault().sport_type.ToLower() == "all")
            var settingAll = betSettings.FirstOrDefault(x => x.sport_type.ToLower() == "all");
            if (settingAll != null)
            {
                betSettings.Clear();
            ...
            //max_payout_per_match =  betSetting.max_bet_per_match * 8
            betSettings = betSettings.Select(betSetting =>
            {
                betSetting.max_payout_per_match = betSetting.max_bet_per_match * 8;
                return betSetting;
            }).ToList();
            ...
            var result = await _gameApiService._Saba2API.SetMemberBetSetting(requestData);
            _logger.LogDebug("Show result : {result}", result);
            if (result.error_code != (int)ResponseCode.Success)
            {
        ...
        {
            return _gameApiService._Saba2API.GetMaintenanceTime(new());
        }
        #endregion
        ...
        /// <returns></returns>
        public async Task<int> PostSabaRecord(SABA_Game_Record recordData)
        {
            //waiting單、running單且有settlement_time 排除   SABA改單規則 狀態會改回running 但會有結算時間 只抓最後結算單讓他變一般改單流程
            recordData.BetDetails = recordData.BetDetails.Where(x => !(x.ticket_status == "running" && x.settlement_time.HasValue) && x.ticket_status != "waiting" && x.vendor_member_id.Contains(Config.OneWalletAPI.Prefix_Key)).ToList();
                ...
                using (NpgsqlConnection conn = new NpgsqlConnection(Config.OneWalletAPI.DBConnection.BetLog.Master))
                {
                    ...
                    using (var tran = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            sw.Restart();
                            string club_id;
                            club_id = group.Key.Substring(Config.CompanyToken.SABA2_Operatorid.Length + 4);
                            ...
                            foreach (SABA_BetDetails r in group)//loop club id bet detail
                            {
                                ...
                                r.club_id = club_id;
                                r.franchiser_id = (memberWalletData !=null ? memberWalletData.Franchiser_id : "");
                                //GMT-4 調整成GMT+8
                                ...
                                r.partition_time = Convert.ToDateTime(r.transaction_time);
                                r.report_time = reportTime;

                                await Calculate(conn, tran, r);
                                ...
                                dic[summaryTime].Add(r.partition_time.ToString("yyyy-MM-dd HH:mm"));
                            }
                            foreach (var item in dic)
                            {
                                foreach (var subItem in item.Value)
                                {
                                    var key = nameof(Platform.SABA) + $"{RedisCacheKeys.TpBetSummaryTime}:{item.Key}";
                            ...
                            {
                                postResult += await _sabaDbService.PostSabaRecord(conn, tran, betDetailData);
                            }
                            if (betDetailDataRunning.Count > 0)
                            {
                                ...
                                postResult += await _sabaDbService.PostSabaRecord(conn, tran, betDetailDataRunning);
                            }
                        ...
                        catch (Exception ex)
                        {
                            ...
                            if (_env.EnvironmentName != "PRD" && (ex.Message == "沒有會員id" || ex.Message == "No saba2 user"))
                            {
                                _logger.LogDebug("Run saba2 record group: {key} exception EX : {ex}  MSG : {Message} ", group.Key, ex.GetType().FullName.ToString(), ex.Message.ToString());
                            }
                            else
                            {
                                _logger.LogError("Run saba2 record group: {key} exception EX : {ex}  MSG : {Message} Error Line : {errorFile}.{errorLine} ", group.Key, ex.GetType().FullName.ToString(), ex.Message.ToString(), errorFile, errorLine);
                            }
                        }
                    }
        ...
        //public async Task<int> PostSabaRecordback(SABA_Game_Record recordData)
        //{
        //    //waiting單、running單且有settlement_time 排除
        //    recordData.BetDetails = recordData.BetDetails.Where(x => !(x.ticket_status == "running" && x.settlement_time.HasValue) && x.ticket_status != "waiting").ToList();
        ...
        //                    var gameUser = await _commonService._gamePlatformUserService.GetSingleGamePlatformUserAsync(club_id, Platform.SABA2);
        //                    if (gameUser == null || gameUser.game_user_id != group.Key)
        //                    {
        //                        throw new Exception("No saba2 user");
        //                    }
        //                    //彙總注單
        ...
        /// 會總遊戲商帳務
        /// </summary>
        /// <param name="sabaReportData"></param>
        /// <returns></returns>
        public async Task<ResCodeBase> PostGameReport(SABA_GetFinancialReportData sabaReportData)
        {
            ...
            reportData.platform = nameof(Platform.SABA2);
        ...
        {
            ResCodeBase res = new ResCodeBase();
            IEnumerable<dynamic> sabaReportData = await _sabaDbService.SumSabaBetRecordDaily(reportDate, Config.CompanyToken.SABA2_Operatorid);
            ...
            reportData.total_win = reportData.total_bet + reportData.total_netwin;
            reportData.platform = nameof(Platform.SABA2);
            await _gameReportDBService.DeleteGameReport(reportData);
            await _gameReportDBService.PostGameReport(reportData);
            return res;
        ...
        #endregion
        private async Task Calculate(NpgsqlConnection conn, IDbTransaction tran, SABA_BetDetails r)
        {
            //處理聯賽主客對資訊串關顯示內容
            ...
            {
                //先查新表在查舊表
                IEnumerable<SABA_BetDetails> runningRecord = await _sabaDbService.GetSabaV2Record(r);
                if (runningRecord == null && !runningRecord.Any())
                    runningRecord = await _sabaDbService.GetSabaRecord(r);
            ...
            await Settle(conn, tran, r);

        }
        private async Task Settle(NpgsqlConnection conn, IDbTransaction tran, SABA_BetDetails r)
        {
            ...
            switch (r.ticket_status)
            {
                ...
                case "draw":
                    ...
                    {
                        //查新表沒有在查舊表
                        Refoundresults = await _sabaDbService.GetSabaV2Record(conn, tran, r);
                        if (Refoundresults == null && !Refoundresults.Any())
                            Refoundresults = await _sabaDbService.GetSabaRecord(conn, tran, r);

                        if (Refoundresults != null && Refoundresults.Any())
                        {
                            //有歷史訂單紀錄，沖銷上一筆
                            SABA_BetDetails lastRecord = Refoundresults.OrderByDescending(r => r.last_version_key).First();
                            decimal? preWinLose = lastRecord.pre_winlost_amount;
                            decimal? preBet = lastRecord.pre_stake;
                ...
                case "half lose":
                    ...
                    if (WinloseResults.Count() != 1) //沒有未結算單
                    {
                        ...
                        else
                        {
                            ...
                            {
                                //馬來盤實投量要計算實際扣款額度
                                r.turnover += Math.Abs((r.stake * r.odds).GetValueOrDefault());
                            }
                            else
                            {
                                r.turnover += r.stake;
                    ...
                    else
                    {
                        ...
                        r.winlost_amount -= preWinLose;
                        r.stake -= preBet;
                        r.turnover = Math.Abs(preWinLose);
        ...
        private async Task<int> RepairSaba(RepairBetSummaryReq RepairReq)
        {
            ...
            SABA_GetBetDetailByTimeframe_Res result = await _gameApiService._Saba2API.GetBetDetailByTimeframe(req);
            var postResult = 0;
            if (result.Data.BetDetails.Count > 0)
            {
                postResult += await PostSabaRecord(result.Data);
            }
            await CreateGameReportFromBetRecord(RepairReq.StartTime);
            return postResult;
        }
        public async Task<int> SabaRecordPaser(IEnumerable<dynamic> saba_records)
        {
            var result = await GetBetTypeCache(nameof(Platform.SABA2));
        ...
        public async Task<bool> RecordSummary(Platform platform, DateTime reportDatetime, DateTime startTime, DateTime endTime)
        {
            ...
            var summaryRecords = await _sabaDbService.SummaryGameRecord(reportDatetime, startTime, endTime);
            ...
            var userlist = summaryRecords.Select(x => x.userid.Substring(Config.CompanyToken.SABA2_Operatorid.Length + 4)).Distinct().ToList();
            ...
            {
                if (!userWalletList.TryGetValue(summaryRecord.Key.Substring(Config.CompanyToken.SABA2_Operatorid.Length + 4), out var userWallet)) continue;
                ...
                summaryData.Game_id = nameof(Platform.SABA);
