...
using RCG3Model = ThirdPartyWallet.Share.Model.Game.RCG3.RCG3;
using RCG3setup = ThirdPartyWallet.Share.Model.Game.RCG3.RCG3;


namespace H1_ThirdPartyWalletAPI.Service.Game.RCG3
{
    public interface IRCG3InterfaceService : IGameInterfaceService
    {
        Task<BaseResponse<GetBetRecordListResponse>> CallRCG3Record(GetBetRecordListRequest request);
        Task<int> PostRcg3RecordDetail(List<RCG3BetRecord> recordData, string systemCode, string webId);
    ...
    public class RCG3_InterfaceService : IRCG3InterfaceService
    {
        private readonly ILogger<RCG3_InterfaceService> _logger;
        private readonly ICommonService _commonService;
        private readonly IWebHostEnvironment _env;
        private readonly ISummaryDBService _summaryDBService;
        private readonly IDBService _dbService;
        private readonly IRCG3ApiService _gameApiService;
        private readonly IRCG3DBService _rcg3DBService;
        ...
        public RCG3_InterfaceService(ILogger<RCG3_InterfaceService> logger,
            ICommonService commonService,
            IWebHostEnvironment env,
            IRCG3ApiService gameaApiService,
            ISummaryDBService summaryDBService,
            IDBService dbService,
            IRCG3DBService rcg3DBService,
        ...
        {
            if (!RCG3setup.Currency.ContainsKey(userData.Currency))
            {
            ...
            try
            {
                var req = new RCG3Model.RCG_CreateOrSetUser()
                {
                    ...
                    openGameList = "ALL",
                    currency = RCG3setup.Currency[userData.Currency],
                    language = RCG3setup.lang.TryGetValue(request.GameConfig["lang"], out var lang) ? lang : RCG3setup.lang["en-US"],
                    h1SHIDString = request.GameConfig["unitHID"],
                };

                var response = await _gameApiService.CreateOrSetUser(req);
                ...
                gameUser.game_platform = Platform.RCG3.ToString();
                return gameUser;
            }
            catch (Exception ex)
            {
                _logger.LogError("RCG3建立會員失敗 Msg: {Message}", ex.Message);
                throw new ExceptionMessage(ResponseCode.Fail, "RCG3 " + ex.Message.ToString());
            }
        }
        ...
        public async Task<MemberBalance> GetGameCredit(Platform platform, GamePlatformUser platform_user)
        {
            ...
            try
            {
                var responseData = await _gameApiService.GetBalance(new RCG3Model.RCG_GetBalance
                {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("RCG3餘額取得失敗 Msg: {Message}", ex.Message);
            }

            Balance.Wallet = nameof(Platform.RCG3);
            return Balance;
        }
        /// <summary>
        /// 踢線
        ...
        public async Task<bool> KickUser(Platform platform, GamePlatformUser platform_user)
        {
            try
            {
                var req = new RCG3Model.RCG_KickOut()
                {
                    memberAccount = platform_user.game_user_id
                };

                var response = await _gameApiService.KickOut(req);

                if (response.msgId != 0) throw new(response.message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "RCG3登出會員失敗 Msg: {Message}", ex.Message);
            }
            return true;
        }
        ...
        public async Task<string> Deposit(GamePlatformUser platform_user, Wallet walletData, WalletTransferRecord RecordData)
        {
            try
            {
                ...
                var responseData = await _gameApiService.Deposit(new RCG3Model.RCG_Deposit
                {
                    memberAccount = platform_user.game_user_id,
                ...
                if (responseData.msgId != 0)
                {
                    ...
                    _logger.LogError("RCG3 Deposit: {Message}", responseData.message);
                }
                else
                {
            ...
            catch (TaskCanceledException ex)
            {
                ...
                _logger.LogError("RCG3 TransferIn Timeout ex : {ex}", ex);
            }
            catch (Exception ex)
            {
                RecordData.status = nameof(WalletTransferRecord.TransferStatus.pending);
                _logger.LogError("RCG3 Deposit: {Message}", ex.Message);
            }
        ...
        public async Task<string> Withdraw(GamePlatformUser platform_user, Wallet walletData, WalletTransferRecord RecordData)
        {
            try
            {
                var responseData = await _gameApiService.Withdraw(new RCG3Model.RCG_Withdraw
                {
                    memberAccount = platform_user.game_user_id,
                    transactionId = RecordData.id.ToString(),
                    transctionAmount = RecordData.amount,
                ...
                {
                    RecordData.status = nameof(WalletTransferRecord.TransferStatus.fail);
                    _logger.LogError("RCG3 Withdraw : {ex}", responseData.message);
                }
                else
                {
                    RecordData.status = nameof(WalletTransferRecord.TransferStatus.success);
            ...
            {
                RecordData.status = nameof(WalletTransferRecord.TransferStatus.pending);
                _logger.LogError("RCG3 TransferOut Timeout ex : {ex}", ex);
            }
            catch (Exception ex)
            {
                RecordData.status = nameof(WalletTransferRecord.TransferStatus.pending);
                _logger.LogError("RCG3 Withdraw : {ex}", ex.Message);
            }
            return RecordData.status;
        }
        ...
        {
            RCG3setup.lang.TryGetValue(request.GameConfig["lang"], out var lang);

            RCG3Model.RCG_Login UrlRequest = new RCG3Model.RCG_Login
            {
                memberAccount = platformUser.game_user_id,
                backUrl = request.GameConfig["lobbyURL"],
                lang = lang ?? RCG3setup.lang["en-US"],
            ...
            try
            {
                var token_res = await _gameApiService.Login(UrlRequest);
            ...
            {
                throw new ExceptionMessage((int)ResponseCode.GetGameURLFail, MessageCode.Message[(int)ResponseCode.GetGameURLFail] + "|" + "RCG3: " + ex.Message.ToString());
            }
        }
        /// <summary>
        /// 確認交易紀錄
        ...
        public async Task<CheckTransferRecordResponse> CheckTransferRecord(WalletTransferRecord transfer_record)
        {
            ...
            var Reuslt = await _gameApiService.GetTransactionLog(new RCG3Model.RCG_GetTransactionLog
            {
                transactionId = transfer_record.id.ToString()
            });
            if (Reuslt.data.status == 200)
            {
                if (transfer_record.target == nameof(Platform.RCG3))//轉入直接改訂單狀態為成功
                {
                    CheckTransferRecordResponse.LockCreditChange = -transfer_record.amount;

                }
                else if (transfer_record.source == nameof(Platform.RCG3))
                {
            ...
            {
                if (transfer_record.target == nameof(Platform.RCG3))//轉入直接改訂單狀態為失敗
                {
                    CheckTransferRecordResponse.CreditChange = transfer_record.amount;
                    CheckTransferRecordResponse.LockCreditChange = -transfer_record.amount;
        ...
        public async Task<dynamic> GetBetRecords(GetBetRecordReq RecordReq)
        {
            ...
            {
                var results = await _rcg3DBService.GetRCG3RecordByReportTimeV2(createTime, RecordReq.ReportTime, summary.Game_type, Config.OneWalletAPI.Prefix_Key + RecordReq.ClubId);
            ...
            if (batRecords.Count == 0)
            {
                foreach (var createTime in createtimePair)
                {
                    batRecords.AddRange(await _rcg3DBService.GetRCG3RecordsBySummary(new()
                    {
        ...
        public async Task<string> GameDetailURL_OLD(GetBetDetailReq RecordDetailReq)
        {
            var rcgRunNo = await _rcg3DBService.GetRCG3RunNoById(long.Parse(RecordDetailReq.record_id), RecordDetailReq.ReportTime);
            if (rcgRunNo == null)
            {
                throw new Exception("no data");
            }

            var requestData = new GetOpenListRequest()
            {
                GameDeskID = rcgRunNo.desk,
                ActiveNo = rcgRunNo.activeNo,
                RunNo = rcgRunNo.runNo,
                Date = rcgRunNo.reportdt
            };

            try
            {
                var responseData = await _gameApiService.GetOpenList(requestData);
                if (responseData.msgId != 0)
                {
                    _logger.LogInformation("RCG3 GameDetailURL 失敗 GameDeskID:{GameDeskID}, ActiveNo:{ActiveNo}, RunNo:{RunNo}, Date:{Date}, msgId: {msgId}, message: {message}",
            ...
            {
                _logger.LogError("RCG3 GameDetailURL 例外 GameDeskID:{GameDeskID}, ActiveNo:{ActiveNo}, RunNo:{RunNo}, Date:{Date} Ex: {Message}",
                    requestData.GameDeskID, requestData.ActiveNo, requestData.RunNo, requestData.Date, ex.Message.ToString());
        ...
        public async Task<string> GameDetailURL(GetBetDetailReq RecordDetailReq)
        {
            var requestData = new SingleRecordWithGameResultRequest()
            {
                ...
                Lang = RCG3setup.lang.TryGetValue(RecordDetailReq.lang, out var lang) ? lang : RCG3setup.lang["zh-CN"]
            };

            try
            {
                var responseData = await _gameApiService.SingleRecordWithGameResult(requestData);
                if (responseData.msgId != 0)
                {
                    _logger.LogInformation("RCG3 SingleRecordWithGameResultRequest 失敗 id:{id}, msgId: {msgId}, message: {message}", RecordDetailReq.record_id, responseData.msgId, responseData.message);
                    return (responseData.msgId).ToString() + ", " + responseData.message;
                }
            ...
            {
                _logger.LogInformation("RCG3 SingleRecordWithGameResultRequest 失敗 id:{id}, Ex: {Message}", RecordDetailReq.record_id, ex.Message.ToString());
                return ex.Message.ToString();
            }
        }
        ...
        /// <param name="request"></param>
        /// <returns></returns>
        public async Task<BaseResponse<GetBetRecordListResponse>> CallRCG3Record(GetBetRecordListRequest request)
        {
            return await _gameApiService.GetBetRecordList(request); ;
        }
        ...
        public async Task<string> RepairGameRecord(RepairBetSummaryReq RepairReq)
        {
            ...
            List<RCG3BetRecord> betRecords = new List<RCG3BetRecord>();
            var data = await _gameApiService.GetBetRecordListByDateRange(response);
            ...
            if (data.data.total != 0)
            {
                ...
                for (int i = 2; i < (double)data.data.total / 1000; i++)
                {
                    ...
                    data = await _gameApiService.GetBetRecordListByDateRange(response);
                    betRecords.AddRange(data.data.dataList);
                }

                int postRcg3Result = await PostRcg3RecordDetail(betRecords, data.data.systemCode, data.data.webId);
                postResult += postRcg3Result;
                #region 重產匯總帳
                await SummaryW1Report(RepairReq.StartTime, RepairReq.EndTime);
                #endregion 重產匯總帳   
            }

            return $"Game: RCG3 新增資料筆數: {postResult}";
            ...
            //if (firstOne == null)
            //{
            //    string errormsg = $"RCG3 Cannot find the previous data, it may be over {offset} hours.";
            ...
            //{
            //    // 請求 RCG 拉單
            //    var RecordResponse = await _gameApiService.GetBetRecordList(new()
            //    {
            //        maxId = first,
            //        rows = _limit
            //    });
        ...
        public async Task<ResCodeBase> SetLimit(SetLimitReq request, GamePlatformUser gameUser, Wallet memberWalletData)
        {
            ...
            var Rcg3res = await CreateGameUser(ForwardGameReq, memberWalletData);
            if (gameUser == null)
            {
                await _commonService._gamePlatformUserService.PostGamePlatformUserAsync(Rcg3res);
            }
        ...
        {
            return _gameApiService.HelloWorld();
        }

        #endregion

        #region GameRecordService
        public async Task<int> PostRcg3RecordDetail(List<RCG3BetRecord> betInfos, string systemCode, string webId)
        {
            var sw = System.Diagnostics.Stopwatch.StartNew();
            if (!betInfos.Any()) return 0;

            betInfos = betInfos.Where(x => x.memberAccount.ToLower().StartsWith(_prefixKey.ToLower())).ToList();
            var existsPK = new HashSet<long>();
            if (betInfos.Any())
            {
                existsPK = (await _rcg3DBService.GetRCG3RealIDByReportDT(betInfos.Min(b => b.reportDT), betInfos.Max(b => b.reportDT)))
                           .Select(b => b.real_id) // Directly select real_id
                           .ToHashSet();
            }
                ...
                foreach (var group in linqRes)
                {
                    using (var tran = await conn.BeginTransactionAsync())
                    {
                        try
                        {
                            ...
                            // 注單明細
                            List<RCG3BetRecord> betDetailData = new List<RCG3BetRecord>();
                            // t_summary_bet_record_mapping
                            var summaryBetRecordMappings = new HashSet<t_summary_bet_record_mapping>();
                            foreach (RCG3BetRecord item in group)
                            {
                                // 排除重複注單
                                if (!existsPK.Add(item.id)) continue;
                            ...
                            _logger.LogDebug("insert RCG3 record member: {group}, count: {count}", group.Key,
                                betDetailData.Count);


                            if (betDetailData.Count > 0)
                            {
                                postResult += await _rcg3DBService.PostRCG3Record(conn, tran, betDetailData);
                            }
                            ...
                            {
                                var key = $"RCG3{RedisCacheKeys.BetSummaryTime}:{item.Key}";
                        ...
                        catch (Exception ex)
                        {
                            ...
                            _logger.LogError("Run RCG3 record group: {key} exception EX : {ex}  MSG : {Message} Error Line : {errorLine}",
        ...
        public async Task SummaryW1Report(DateTime startDateTime, DateTime endDateTime)
        {
            while (true)
            {
                ...
                _logger.LogDebug("Create RCG3 game W1 report time {datetime}", reportTime);
                var (totalCount, totalBetValid, totalWin) = await _rcg3DBService.SumRCG3BetRecordByReportDT(reportTime, reportTime.AddHours(1));

                GameReport reportData = new();
                reportData.platform = nameof(Platform.RCG3);
        ...
        private async Task Calculate(NpgsqlConnection conn, IDbTransaction tran, RCG3BetRecord r)
        {
            // 真人注單有改牌邏輯，需先儲存原始資料
            r.real_id = r.id;
            r.pre_bet = r.bet;
            ...
            switch (r.status)
            {
                case (int)RCG3setup.BetStatusEnum.Cancel:
                case (int)RCG3setup.BetStatusEnum.Change:
                    // 優先嘗試從主要來源獲取舊紀錄
                    var oldRecords = await _rcg3DBService.GetRCG3PreRecordById(tran, r.id, r.reportDT)
                 ?? await _rcg3DBService.GetRCG3PreRecordById_old(tran, r.id, r.reportDT)
                 ?? new List<RCG3PreRecordDBResponse>();
        ...
        public async Task<bool> RecordSummary(Platform platform, DateTime reportDatetime, DateTime startTime, DateTime endTime)
        {
            ...
            var summaryRecords = await _rcg3DBService.SummaryGameRecord(reportDatetime, startTime, endTime);
            ...
            foreach (var summaryRecord in Groupsummary)
            {
                ...
                summaryData.Game_id = nameof(Platform.RCG3);
            ...
            sw2.Stop();
            _logger.LogInformation("RCG3 summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
        ...
        private async Task<(DateTime StartTime, DateTime EndTime)> GetRecordSummaryDateTime(DateTime reportTime)
        {
            ...
            var redisKey = $"RCG3{RedisCacheKeys.BetSummaryTime}:{reportTime.ToString("yyyy-MM-dd HH:mm")}";
