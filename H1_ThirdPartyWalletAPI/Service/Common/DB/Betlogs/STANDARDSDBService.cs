...
{
    public interface ISTANDARDSDBService
    {
        Task<int> PostSTANDARDSRecord(NpgsqlConnection conn, IDbTransaction tran, IEnumerable<BetlogResponse.Page_Info> betLogs);
        Task<List<BetlogResponse.Page_Info>> GetSTANDARDSRecordsBytime(DateTime createtime, DateTime report_time, string club_id);
        ...
        Task<IEnumerable<(int count, decimal win, decimal bet, decimal jackpot, string userid, int game_type, DateTime createtime)>> SummaryGameRecord(DateTime reportTime, DateTime startTime, DateTime endTime);
            Task<(int totalCount, decimal totalBetValid, decimal totalWin, decimal totalnetwin)> SumSTANDARDSBetRecordByBetTime(DateTime start, DateTime end);
    }
    public class STANDARDSDBService : BetlogsDBServiceBase, ISTANDARDSDBService
    {
        public STANDARDSDBService(ILogger<STANDARDSDBService> logger, IBetLogsDbConnectionStringManager connectionStringManager) : base(logger, connectionStringManager)
        {
        }
        ...
        /// <returns></returns>
        public async Task<int> PostSTANDARDSRecord(NpgsqlConnection conn, IDbTransaction tran, IEnumerable<BetlogResponse.Page_Info> betLogs)
        {
        ...
        {
            var sql = $@"CREATE TEMPORARY TABLE temp_t_STANDARDS_bet_record_{tableGuid:N} 
                            ( LIKE t_STANDARDS_bet_record  INCLUDING ALL );";
        ...
        private async Task<ulong> BulkInsertToTempTable(IDbTransaction tran, Guid tableGuid, IEnumerable<BetlogResponse.Page_Info> records)
        {
            ...
            await using var writer =
                await npTran.Connection.BeginBinaryImportAsync(
                    @$"COPY temp_t_STANDARDS_bet_record_{tableGuid:N} (bet_id, round, gamecode, account, currency, bet_amount, bet_valid_amount, pay_off_amount,  
        ...
        {
            var sql = @$"INSERT INTO t_STANDARDS_bet_record
                        ...
                        FROM temp_t_STANDARDS_bet_record_{tableGuid:N} AS  temp
                        WHERE NOT EXISTS 
                        (
                            SELECT NULL
                            FROM public.t_STANDARDS_bet_record
        ...
        /// <returns></returns>
        public async Task<List<BetlogResponse.Page_Info>> GetSTANDARDSRecordsBytime(DateTime createtime, DateTime report_time, string club_id)
        {
            try
            {
                var sql = @"SELECT bet_id, round, gamecode, account, bet_amount, bet_valid_amount, pay_off_amount, jp_win,
                                bet_time, pay_off_time, status ,partition_time  FROM public.t_STANDARDS_bet_record 
        ...
        /// <returns></returns>
        public async Task<(int totalCount, decimal totalBetValid, decimal totalWin, decimal totalnetwin)> SumSTANDARDSBetRecordByBetTime(DateTime start, DateTime end)
        {
            var sql = @"SELECT 
