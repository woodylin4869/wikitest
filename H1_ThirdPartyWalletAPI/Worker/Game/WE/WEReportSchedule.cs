...
{
    public class WEReportSchedule : IInvocable
    {
        private readonly ILogger<WEReportSchedule> _logger;
        private readonly IWEInterfaceService _apiInterfaceService;
        private readonly ISystemParameterDbService _systemParameterDbService;

        public WEReportSchedule(ILogger<WEReportSchedule> logger,
            IWEInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke WEReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "WEReportSchedule";

                // 取得同步 WE 每小時遊戲匯總報表的時間基準
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("WE report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run WE report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}