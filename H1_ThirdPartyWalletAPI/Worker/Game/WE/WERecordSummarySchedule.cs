...
{ 
    public class WERecordSummarySchedule : IInvocable
    {
        private readonly ILogger<WERecordSummarySchedule> _logger;
        ...
        public WERecordSummarySchedule(ILogger<WERecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke WERecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "WERecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("WE summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.WEBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.WE, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("WE summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run WERecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}