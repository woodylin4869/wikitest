...
{
    public class SPLUSRecordSchedule : IInvocable
    {
        private readonly ILogger<SPLUSRecordSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "SPLUSRecordSchedule";

        private readonly TimeSpan GAP_TIME = TimeSpan.FromMinutes(-5);
        private readonly TimeSpan RANGE_OFFSET = TimeSpan.FromMinutes(1);

        public SPLUSRecordSchedule(ILogger<SPLUSRecordSchedule> logger, ISPLUS_InterfaceService apiInterfaceService, ISPLUSApiService SPLUSApiService, IDBService dbService, ISystemParameterDbService systemParameterDbService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke SPLUSRecordSchedule on time : {time}", DateTime.Now.ToLocalTime());
            ...
            {
                _logger.LogInformation("SPLUS record stop time: {time}", parameter.value);
            ...
            try
            {
                BetlogRequest req = new BetlogRequest()
                {
                    ...
                    end_time = lastEndTime.AddMinutes(1).ToString("yyyy-MM-ddTHH:mm:ss"),
                    page = 1,
                    page_size = 2000
                };
                ...
                await _apiInterfaceService.PostSPLUSRecord(res);
            }
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run SPLUS record schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        ...
        private async void TriggerFailOver(t_system_parameter parameter, TimeSpan offTimeSpan)
        {
            ...
            Platform PlatformSPLUS = Platform.SPLUS;

            var failoverReq = new PullRecordFailoverWithTimeOffset()
            {
                platform = PlatformSPLUS,
                repairParameter = parameter.value, // 已經是新的時間
                delay = TimeSpan.FromMinutes(5),
                OffTimeSpan = -offTimeSpan         // 轉換成負值，是要找回起始時間
            };

            await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{PlatformSPLUS}", failoverReq);
        }
    }
}
