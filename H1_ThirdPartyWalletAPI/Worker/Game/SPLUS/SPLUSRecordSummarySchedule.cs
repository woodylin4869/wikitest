...
{
    public class SPLUSRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<SPLUSRecordSummarySchedule> _logger;
        ...
        public SPLUSRecordSummarySchedule(ILogger<SPLUSRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke SPLUSRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "SPLUSRecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "SPLUS-5分鐘匯總排程時間",
                        description = "SPLUS-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("SPLUS summary record stop time: {time}", parameter.value);
                ...
                var redisKey = nameof(Platform.SPLUS) +  $"{RedisCacheKeys.BetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.SPLUS, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("SPLUS summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run SPLUS RecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
