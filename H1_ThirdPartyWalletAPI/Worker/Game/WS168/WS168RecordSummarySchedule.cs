...
namespace H1_ThirdPartyWalletAPI.Worker.Game.WS168
{
    public class WS168RecordSummarySchedule : IInvocable
    {
        private readonly ILogger<WS168RecordSummarySchedule> _logger;
        ...
        public WS168RecordSummarySchedule(ILogger<WS168RecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke WS168RecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "WS168RecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "WS168-5分鐘匯總排程時間",
                        description = "WS168-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("WS168 summary record stop time: {time}", parameter.value);
                ...
                var redisKey = nameof(Platform.WS168) + $"{RedisCacheKeys.BetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.WS168, Convert.ToDateTime(parameter.value), startTime.Value.ToLocalTime(), endTime.Value.ToLocalTime());
                sw.Stop();
                _logger.LogInformation("WS168 summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run WS168 RecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
