...
{
    public class WS168RecordFailoverSchedule : IInvocable
    {
        private readonly ILogger<WS168RecordFailoverSchedule> _logger;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ICacheDataService _cacheDataService;

        public WS168RecordFailoverSchedule(ILogger<WS168RecordFailoverSchedule> logger, IRepairBetRecordService repairBetRecordService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                var req = await _cacheDataService.ListPopAsync<PullRecordFailoverWithTimeOffset>($"{RedisCacheKeys.PullRecordFailOver}:{Platform.WS168}");

                if (req is null) return;

                if (req.requestTime.Add(req.delay) >= DateTime.Now)
                {
                    await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{Platform.WS168}", req);
                    return;
                }

                _logger.LogInformation("Execute WS168 PullRecordFailOver {value}", req.repairParameter);
                ...
                {
                    game_id = Platform.WS168.ToString(),
