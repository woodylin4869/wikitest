...
{
    public class GeminiRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<GeminiRecordSummarySchedule> _logger;
        ...
        public GeminiRecordSummarySchedule(ILogger<GeminiRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke GEMINIRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "GEMINIRecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "GEMINI-5分鐘匯總排程時間",
                        description = "GEMINI-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("Gemini summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.GeminiBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.GEMINI, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("GEMINI summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run GeminiRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
