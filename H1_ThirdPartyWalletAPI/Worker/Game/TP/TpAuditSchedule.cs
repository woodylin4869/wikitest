...
public class TpAuditSchedule : IInvocable
{
    private readonly ILogger<TpAuditSchedule> _logger;
    ...
    private const string SYSTEM_PARAMETERS_KEY = "TPAuditSchedule";

    public TpAuditSchedule(ILogger<TpAuditSchedule> logger, IDBService dbService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService, IGameReportDBService gameReportDBService)
    {
        _logger = logger;
    ...
    public async Task Invoke()
    {
        ...
        _logger.LogInformation("Invoke TPAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
        try
        {
            ...
            else
            {
                ...
                {
                    _logger.LogInformation("tp audit schedule stop time: {time}", parameter.value);
                ...
                {
                    _logger.LogInformation("return tp audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                    return; // 時間不變就結束排程
                }
            }
            var compare = await CompareTPAndW1Report(Convert.ToDateTime(parameter.value), Convert.ToDateTime(parameter.value).AddHours(1).AddSeconds(-1));
            //若TP及W1匯總帳有差異則進行補單
            if (!compare)
            {
                await _repairBetRecordService.RepairGameRecord(new RepairBetSummaryReq()
                {
                    game_id = Platform.TP.ToString(),
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError("Run tp audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
        }
    }
    ...
    /// <returns></returns>
    private async Task<bool> CompareTPAndW1Report(DateTime startTime, DateTime endTime)
    {
        // 遊戲商的匯總帳
        var tpReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.TP,
            ((int)GameReport.e_report_type.FinancalReport).ToString(),
            startTime,
            endTime);

        // 轉帳中心的匯總帳
        var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.TP,
