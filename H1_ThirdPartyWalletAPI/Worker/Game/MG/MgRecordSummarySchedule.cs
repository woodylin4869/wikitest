...
{
    public class MgRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<MgRecordSummarySchedule> _logger;
        ...
        public MgRecordSummarySchedule(ILogger<MgRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke MGRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "MgRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("MG summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.MgBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                var sw = System.Diagnostics.Stopwatch.StartNew();
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.MG, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("MG summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run MGRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
