...
namespace H1_ThirdPartyWalletAPI.Worker.Game.JILI
{
    public class JiliRecordSummarySchedule: IInvocable
    {
        private readonly ILogger<JiliRecordSummarySchedule> _logger;
        ...
        public JiliRecordSummarySchedule(ILogger<JiliRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke JiliRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "JiliRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("Jili summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.JiliBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.JILI, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("Jili summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run JiliRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
