...
namespace H1_ThirdPartyWalletAPI.Worker
{
    ...
    /// 每小時檢查 RTG 注單資料排程
    /// </summary>
    public class RtgAuditSchedule : IInvocable
    {
        private readonly ILogger<RtgAuditSchedule> _logger;
        ...
        public RtgAuditSchedule(ILogger<RtgAuditSchedule> logger, ICommonService commonService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService, IGameReportDBService gameReportDBService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke RtgAuditSchedule on time : {time}", DateTime.Now);
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("rtg audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return rtg audit schedule current Time : {now} report time : {report time} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }
                ...
                // 遊戲商的匯總帳
                var rtgReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.RTG,
                    ((int)GameReport.e_report_type.FinancalReport).ToString(),
                    StartTime,
                    EndTime);
                // 轉帳中心的匯總帳
                var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.RTG,
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run rtg audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
