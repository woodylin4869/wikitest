...
{
    public class OBReportSchedule: IInvocable
    {
        private readonly ILogger<OBReportSchedule> _logger;
        private readonly IOBInterfaceService _apiInterfaceService;
        private readonly ICommonService _commonService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public OBReportSchedule(ILogger<OBReportSchedule> logger,
           IOBInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke OBReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "OBReportSchedule";

                // 取得同步 OB 每小時遊戲匯總報表的時間基準
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("OB report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run OB report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
