...
{
    public class OBAuditSchedule : IInvocable
    {
        private readonly ILogger<OBAuditSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "OBAuditSchedule";
        public OBAuditSchedule(ILogger<OBAuditSchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke OBAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("OB audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return OB audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }
                ...
                {
                    var compare = await CompareOBAndW1Report(Convert.ToDateTime(lastReportTime), Convert.ToDateTime(lastReportTime));
                    ...
                    if (!compare)
                    {
                        ...
                        {
                            game_id = Platform.OB.ToString(),
                            StartTime = Convert.ToDateTime(lastReportTime),
                            EndTime = Convert.ToDateTime(lastReportTime)
                        });
                    }
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run OB audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
        ...
        /// <returns></returns>
        private async Task<bool> CompareOBAndW1Report(DateTime startTime, DateTime endTime)
        {
            // 遊戲商的匯總帳
            var tpReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.OB,
                ((int)GameReport.e_report_type.FinancalReport).ToString(),
                startTime,
                endTime);

            // 轉帳中心的匯總帳
            var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.OB,
