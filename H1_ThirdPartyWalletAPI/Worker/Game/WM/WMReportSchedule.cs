...
{
    public class WMReportSchedule : IInvocable
    {
        private readonly ILogger<WMReportSchedule> _logger;
        private readonly IWMInterfaceService _apiInterfaceService;
        private readonly ICommonService _commonService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public WMReportSchedule(ILogger<WMReportSchedule> logger,
            IWMInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
       {
            ...
            _logger.LogInformation("Invoke WMReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "WMReportSchedule";

                // 取得同步 WM 每小時遊戲匯總報表的時間基準
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("ＷＭ report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run WM report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
