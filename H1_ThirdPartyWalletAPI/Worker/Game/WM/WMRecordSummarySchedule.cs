...
namespace H1_ThirdPartyWalletAPI.Worker.Game.WM
{ 
    public class WMRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<WMRecordSummarySchedule> _logger;
        ...
        public WMRecordSummarySchedule(ILogger<WMRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke WMRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "WMRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("WM summary record stop time: {time}", parameter.value);
                ...
                var redisKey = nameof(Platform.WM) + $"{RedisCacheKeys.BetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.WM, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("WM summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run WMRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}