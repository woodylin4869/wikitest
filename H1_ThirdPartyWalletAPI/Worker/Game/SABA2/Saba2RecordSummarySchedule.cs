...
namespace H1_ThirdPartyWalletAPI.Worker.Game.SABA2
{
    public class Saba2RecordSummarySchedule : IInvocable
    {
        private readonly ILogger<Saba2RecordSummarySchedule> _logger;
        ...
        public Saba2RecordSummarySchedule(ILogger<Saba2RecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke Saba2RecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "Saba2RecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "SABA2-5分鐘匯總排程時間",
                        description = "SABA2-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("SABA2 summary record stop time: {time}", parameter.value);
                ...
                var redisKey = nameof(Platform.SABA) + $"{RedisCacheKeys.BetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                if (startTime == null || endTime == null)
                {
                    ...
                    endTime = Convert.ToDateTime(parameter.value).AddHours(1).AddMilliseconds(-1);
                }
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.SABA2, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("SABA2 summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run Saba2RecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
