...
{
    public class Saba2RecordSchedule : IInvocable
    {
        private readonly ILogger<Saba2RecordSchedule> _logger;
        ...
        public Saba2RecordSchedule(ILogger<Saba2RecordSchedule> logger
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                _logger.LogDebug("Saba2 Record Schedule countdown : {count}", pendingCount);
                //version_key相同時要延遲請求
                if (pendingCount != 0)
                {
                    pendingCount--;
                ...
                if (parameter == null)
                {
                    var last_version_key = await _sabaDbService.GetSabaLastVersionKey(Config.CompanyToken.SABA2_Operatorid);
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("Saba2 record stop time: {time}", parameter.value);
                        await Task.CompletedTask;
                        return;
                    }
                ...
                //}
                SABA_GetBetDetail_Res result = await _gameApiService._Saba2API.GetBetDetail(req);
                ...
                if (result.Data.BetDetails.Count > 0)
                {
                    ...
                    //else
                    await _gameRecordService._Saba2InterfaceService.PostSabaRecord(result.Data);
                }
                // version_key寫回 DB
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run saba2 record schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
