...
{
    public class PSFishRecordSchedule : IInvocable
    {
        private readonly ILogger<PSFishRecordSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "PSFishRecordSchedule";

        private readonly TimeSpan GAP_TIME = TimeSpan.FromMinutes(-5);
        private readonly TimeSpan RANGE_OFFSET = TimeSpan.FromMinutes(1);

        public PSFishRecordSchedule(ILogger<PSFishRecordSchedule> logger, IPsInterfaceService apiInterfaceService, IPsApiService PSApiService, IDBService dbService, ISystemParameterDbService systemParameterDbService, IOptions<PsConfig> options)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PSFishRecordSchedule on time : {time}", DateTime.Now.ToLocalTime());
            ...
            try
            {
                ...
                var resPK = res.Select(x => new { x.sn, x.createtime }).ToHashSet();
                ...
                foreach (var game_type in type_array)
                {
                    ...
                    foreach (var dateEntry in betLogs)
                    {
                        foreach (var memberEntry in dateEntry.Value)
                        {
                            ...
                            {
                                if (resPK.Add(new { record.sn, record.createtime }))
                                {
