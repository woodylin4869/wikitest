...
{
    public class PSReportSchedule: IInvocable
    {

        private readonly ILogger<PSReportSchedule> _logger;
        private readonly IPsInterfaceService _apiInterfaceService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public PSReportSchedule(ILogger<PSReportSchedule> logger,
            IPsInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PSReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "PSReportSchedule";

                // 取得同步 PS 每小時遊戲匯總報表的時間基準
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("PS report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run PS report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
