...
{
    public class PSRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<PSRecordSummarySchedule> _logger;
        ...
        public PSRecordSummarySchedule(ILogger<PSRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PSRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "PSRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("PS summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.PSBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.PS, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("PS summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
