...
{
    public class XgAuditSchedule : IInvocable
    {
        private readonly ILogger<XgAuditSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "XgAuditSchedule";

        public XgAuditSchedule(ILogger<XgAuditSchedule> logger, IDBService dbService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService, IGameReportDBService gameReportDBService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke XgAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("XgAuditSchedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return XgAuditSchedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }

                var compare = await CompareXGAndW1Report(Convert.ToDateTime(parameter.value), Convert.ToDateTime(parameter.value).AddHours(1).AddSeconds(-1));
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run XgAuditSchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
        ...
        /// <returns></returns>
        private async Task<bool> CompareXGAndW1Report(DateTime startTime, DateTime endTime)
        {
            // 遊戲商的匯總帳
            var tpReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.XG,
                ((int)GameReport.e_report_type.FinancalReport).ToString(),
                startTime,
                endTime);

            // 轉帳中心的匯總帳
            var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.XG,
