...
{
    public class VAReportSchedule: IInvocable
    {

        private readonly ILogger<VAReportSchedule> _logger;
        private readonly IVAInterfaceService _apiInterfaceService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public VAReportSchedule(ILogger<VAReportSchedule> logger,
            IVAInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke VAReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "VAReportSchedule";

                // 取得同步 VA 每小時遊戲匯總報表的時間基準
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("VA report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run VA report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
