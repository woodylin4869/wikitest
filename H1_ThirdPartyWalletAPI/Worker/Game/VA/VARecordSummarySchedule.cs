...
{
    public class VARecordSummarySchedule : IInvocable
    {
        private readonly ILogger<VARecordSummarySchedule> _logger;
        ...
        public VARecordSummarySchedule(ILogger<VARecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke VARecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "VARecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("VA summary record stop time: {time}", parameter.value);
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.VA, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run VARecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
