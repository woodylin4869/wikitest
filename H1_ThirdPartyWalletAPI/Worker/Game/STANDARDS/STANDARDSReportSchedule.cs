...
{
    public class STANDARDSReportSchedule : IInvocable
    {

        private readonly ILogger<STANDARDSReportSchedule> _logger;
        private readonly ISTANDARDS_InterfaceService _apiInterfaceService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public STANDARDSReportSchedule(ILogger<STANDARDSReportSchedule> logger,
            ISTANDARDS_InterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke STANDARDSReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                var key = "STANDARDSReportSchedule";

                // 取得同步 STANDARDS 每小時遊戲匯總報表的時間基準
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "STANDARDS 每小時遊戲匯總報表排程",
                        description = "STANDARDS 紀錄排程時間基準點"
                    };
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("STANDARDS report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run STANDARDS report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
