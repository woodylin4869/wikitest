...
{
    public class STANDARDSRecordFailoverSchedule : IInvocable
    {
        private readonly ILogger<STANDARDSRecordFailoverSchedule> _logger;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ICacheDataService _cacheDataService;

        public STANDARDSRecordFailoverSchedule(ILogger<STANDARDSRecordFailoverSchedule> logger, IRepairBetRecordService repairBetRecordService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                var req = await _cacheDataService.ListPopAsync<PullRecordFailoverWithTimeOffset>($"{RedisCacheKeys.PullRecordFailOver}:{Platform.STANDARDS}");

                if (req is null) return;

                if (req.requestTime.Add(req.delay) >= DateTime.Now)
                {
                    await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{Platform.STANDARDS}", req);
                    return;
                }

                _logger.LogInformation("Execute STANDARDS PullRecordFailOver {value}", req.repairParameter);
