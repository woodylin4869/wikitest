...
{
    public class JokerRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<JokerRecordSummarySchedule> _logger;
        ...
        public JokerRecordSummarySchedule(ILogger<JokerRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke JokerRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "JOKERRecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "JOKER-5分鐘匯總排程時間",
                        description = "JOKER-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("joker summary record stop time: {time}", parameter.value);
                ...
                // 將老虎機、魚機記錄好的 reporttime > time 取出
                var redisKey = $"{RedisCacheKeys.JokerBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.JOKER, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("joker summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run JokerRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
