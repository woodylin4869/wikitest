...
{
    public class JokerAuditSchedule : IInvocable
    {
        private readonly ILogger<JokerAuditSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "JokerAuditSchedule";

        public JokerAuditSchedule(ILogger<JokerAuditSchedule> logger, IDBService dbService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService, IGameReportDBService gameReportDBService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke JokerAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "Joker 每小時遊戲帳務比對排程",
                        description = "Joker 紀錄帳務比對排程時間基準點"
                    };
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("joker audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return joker audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }


                var compare = await CompareJokerAndW1Report(Convert.ToDateTime(parameter.value), Convert.ToDateTime(parameter.value).AddHours(1).AddSeconds(-1));

                //若Joker及W1匯總帳有差異則進行補單
                if (!compare)
                {
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run joker audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
        ...
        /// <returns></returns>
        private async Task<bool> CompareJokerAndW1Report(DateTime startTime, DateTime endTime)
        {
            // 遊戲商的匯總帳
            var tpReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.JOKER,
                ((int)GameReport.e_report_type.FinancalReport).ToString(),
                startTime,
                endTime);

            // 轉帳中心的匯總帳
            var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.JOKER,
