...
{
    public class Rcg2RecordSummarySchedule : IInvocable
    {
        private readonly ILogger<Rcg2RecordSummarySchedule> _logger;
        ...
        public Rcg2RecordSummarySchedule(ILogger<Rcg2RecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke RCG2RecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "RCG2RecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("RCG2 summary record stop time: {time}", parameter.value);
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.RCG2, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run RCG2RecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}