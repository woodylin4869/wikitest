...
{
    public class CRRecordFailoverSchedule : IInvocable
    {
        private readonly ILogger<CRRecordFailoverSchedule> _logger;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ICacheDataService _cacheDataService;

        public CRRecordFailoverSchedule(ILogger<CRRecordFailoverSchedule> logger, IRepairBetRecordService repairBetRecordService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                var req = await _cacheDataService.ListPopAsync<PullRecordFailoverWithTimeOffset>($"{RedisCacheKeys.PullRecordFailOver}:{Platform.CR}");

                if (req is null) return;

                if (req.requestTime.Add(req.delay) >= DateTime.Now)
                {
                    await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{Platform.CR}", req);
                    return;
                }

                _logger.LogInformation("Execute CR PullRecordFailOver {value}", req.repairParameter);
