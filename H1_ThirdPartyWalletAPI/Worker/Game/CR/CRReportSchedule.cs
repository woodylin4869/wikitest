...
{
    public class CRReportSchedule : IInvocable
    {
        private readonly ILogger<CRReportSchedule> _logger;
        private readonly ICRInterfaceService _apiInterfaceService;
        private readonly ISystemParameterDbService _systemParameterDbService;
        public const string SYSTEM_PARAMETERS_KEY = "CRReportSchedule";
        public CRReportSchedule(ILogger<CRReportSchedule> logger,
            ICRInterfaceService apiInterfaceService,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke CRReportSchedule on time : {time}", DateTime.Now);
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("CR report stop time: {time}", parameter.value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run CR report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}