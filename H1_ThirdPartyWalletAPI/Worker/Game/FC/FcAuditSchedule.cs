...
{
    public class FcAuditSchedule : IInvocable
    {
        private readonly ILogger<FcAuditSchedule> _logger;
        ...
        private const string SYSTEM_PARAMETERS_KEY = "FcAuditSchedule";
        private readonly ISystemParameterDbService _systemParameterDbService;

        public FcAuditSchedule(ILogger<FcAuditSchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke FcAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("FC audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return FC audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }
                ...
                {
                    var compare = await CompareFCAndW1Report(Convert.ToDateTime(lastReportTime), Convert.ToDateTime(lastReportTime));
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run FC audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
        ...
        /// <returns></returns>
        private async Task<bool> CompareFCAndW1Report(DateTime startTime, DateTime endTime)
        {
            // 遊戲商的匯總帳
            var tpReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.FC,
                ((int)GameReport.e_report_type.FinancalReport).ToString(),
                startTime,
                endTime);

            // 轉帳中心的匯總帳
            var w1ReportSummary = await _gameReportDBService.GetGameReportSummary(Platform.FC,
