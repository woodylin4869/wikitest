...
namespace H1_ThirdPartyWalletAPI.Worker.Game.FC
{
    public class FcRecordFailoverSchedule : IInvocable
    {
        private readonly ILogger<FcRecordFailoverSchedule> _logger;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ICacheDataService _cacheDataService;

        public FcRecordFailoverSchedule(ILogger<FcRecordFailoverSchedule> logger, IRepairBetRecordService repairBetRecordService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                var req = await _cacheDataService.ListPopAsync<PullRecordFailoverWithTimeOffset>($"{RedisCacheKeys.PullRecordFailOver}:{Platform.FC}");

                if (req is null) return;

                if (req.requestTime.Add(req.delay) >= DateTime.Now)
                {
                    await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{Platform.FC}", req);
                    return;
                }

                _logger.LogInformation("Execute FC PullRecordFailOver {value}", req.repairParameter);
