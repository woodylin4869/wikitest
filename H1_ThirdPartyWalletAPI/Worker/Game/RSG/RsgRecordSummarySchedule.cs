...
{
    public class RsgRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<RsgRecordSummarySchedule> _logger;
        ...
        public RsgRecordSummarySchedule(ILogger<RsgRecordSummarySchedule> logger, 
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke RsgRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "RSGRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("rsg summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.RsgBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.RSG, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("rsg summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}", 
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run RsgRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
