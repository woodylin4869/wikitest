...
{
    public class RGRICHRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<RGRICHRecordSummarySchedule> _logger;
        ...
        public RGRICHRecordSummarySchedule(ILogger<RGRICHRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke RGRICHRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "RGRICHRecordSummarySchedule";
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "RGRICH-5分鐘匯總排程時間",
                        description = "RGRICH-5分鐘匯總排程時間基準點"
                    };
                ...
                {
                    _logger.LogInformation("RGRICH summary record stop time: {time}", parameter.value);
                ...
                // 將老虎機紀錄好的 reporttime > bet_Time(下注時間) 取出

                #region redis with SortedSet 作法

                var redisKey = $"{RedisCacheKeys.RGRICHBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                // SortedSet作法
                ...
                #endregion redis with SortedSet 作法
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.RGRICH, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("RGRICH summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run RGRICHRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}