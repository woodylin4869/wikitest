...
{
    public class RGRICHRecordFailoverSchedule : IInvocable
    {
        private readonly ILogger<RGRICHRecordFailoverSchedule> _logger;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ICacheDataService _cacheDataService;

        public RGRICHRecordFailoverSchedule(ILogger<RGRICHRecordFailoverSchedule> logger, IRepairBetRecordService repairBetRecordService, ICacheDataService cacheDataService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            try
            {
                var req = await _cacheDataService.ListPopAsync<PullRecordFailoverWithTimeOffset>($"{RedisCacheKeys.PullRecordFailOver}:{Platform.RGRICH}");

                if (req is null) return;

                if (req.requestTime.Add(req.delay) >= DateTime.Now)
                {
                    await _cacheDataService.ListPushAsync($"{RedisCacheKeys.PullRecordFailOver}:{Platform.RGRICH}", req);
                    return;
                }

                _logger.LogInformation("Execute RGRICH PullRecordFailOver {value}", req.repairParameter);
