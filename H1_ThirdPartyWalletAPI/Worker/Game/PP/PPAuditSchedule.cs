...
{
    public class PPAuditSchedule : IInvocable
    {
        private readonly ILogger<PPAuditSchedule> _logger;
        private readonly IDBService _dbService;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ISystemParameterDbService _systemParameterDbService;

        private const string SYSTEM_PARAMETERS_KEY = "PPAuditSchedule";

        public PPAuditSchedule(ILogger<PPAuditSchedule> logger, IDBService dbService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PPAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("PP audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return PP audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run PP audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
