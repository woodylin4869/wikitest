...
{
    public class SabaAuditSchedule : IInvocable
    {
        private readonly ILogger<SabaAuditSchedule> _logger;
        ...
        public SabaAuditSchedule(ILogger<SabaAuditSchedule> logger
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke SabaAuditSchedule on time : {time}", DateTime.Now);
            try
            {
                _logger.LogInformation("Run Saba audit schedule on {datetime}", DateTime.Now);
                DateTime dt = DateTime.Parse(DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd"));
                IEnumerable<GameReport> result = await _gameReportDBService.GetGameReport(Platform.SABA, null, dt, dt, 0, 10);
                ...
                //2. 檢查投注次數與輸贏是否一致
                if (FinancialReport.total_count != RecordDailyReport.total_count || FinancialReport.total_netwin != RecordDailyReport.total_netwin)
                {
                    _logger.LogWarning("Audit SABA report fail Date : {date} FinancalReport : {FinancialReport} RecordDailyReport : {RecordDailyReport}"
                    ...
                    req.game_id = nameof(Platform.SABA);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run saba audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
