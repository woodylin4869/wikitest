...
{
    public class Rcg3RecordSummarySchedule : IInvocable
    {
        private readonly ILogger<Rcg3RecordSummarySchedule> _logger;
        ...
        public Rcg3RecordSummarySchedule(ILogger<Rcg3RecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke Rcg3RecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "Rcg3RecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("RCG3 summary record stop time: {time}", parameter.value);
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.RCG3, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run Rcg3RecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}