...
{
    public class IDNRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<IDNRecordSummarySchedule> _logger;
        ...
        public IDNRecordSummarySchedule(ILogger<IDNRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke IDNRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "IDNRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("IDN summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.IDNBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";
                // SortedSet作法
                ...
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.IDN, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("IDN summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run IDNRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}