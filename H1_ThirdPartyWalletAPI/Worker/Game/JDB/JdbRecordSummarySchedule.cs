...
{
    public class JdbRecordSummarySchedule : IInvocable
    {
        private readonly ILogger<JdbRecordSummarySchedule> _logger;
        ...
        public JdbRecordSummarySchedule(ILogger<JdbRecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke JdbRecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "JdbRecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("jdb summary record stop time: {time}", parameter.value);
                ...
                var redisKey = $"{RedisCacheKeys.JdbBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";

                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.JDB, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run JdbRecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}