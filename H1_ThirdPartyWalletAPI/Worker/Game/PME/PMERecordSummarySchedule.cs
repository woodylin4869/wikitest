...
{
    public class PMERecordSummarySchedule : IInvocable
    {
        private readonly ILogger<PMERecordSummarySchedule> _logger;
        ...
        public PMERecordSummarySchedule(ILogger<PMERecordSummarySchedule> logger,
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PMERecordSummarySchedule on time : {time}", DateTime.Now);
            try
            {
                t_system_parameter parameter = null;
                var key = "PMERecordSummarySchedule";
                ...
                {
                    _logger.LogInformation("PME summary record stop time: {time}", parameter.value);
                ...
                // 將注單記錄好的 reporttime > playtime 取出
                var redisKey = $"{RedisCacheKeys.PMEBetSummaryTime}:{Convert.ToDateTime(parameter.value).ToString("yyyy-MM-dd HH:mm")}";

                var sw = System.Diagnostics.Stopwatch.StartNew();
                // 查詢時間寫回 DB
                await _gameInterfaceService.RecordSummary(Platform.PME, Convert.ToDateTime(parameter.value), startTime.Value, endTime.Value);
                sw.Stop();
                _logger.LogInformation("PME summary record 寫入完成時間 {time}, 五分鐘匯總帳時間: {reporttime}, 開始時間: {starttime} 結束時間: {endtime}",
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError("Run PMERecordSummarySchedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
