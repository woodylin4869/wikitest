...
{
    public class PMEAuditSchedule : IInvocable
    {
        private readonly ILogger<PMEAuditSchedule> _logger;
        private readonly IDBService _dbService;
        private readonly IRepairBetRecordService _repairBetRecordService;
        private readonly ISystemParameterDbService _systemParameterDbService;

        private const string SYSTEM_PARAMETERS_KEY = "PMEAuditSchedule";

        public PMEAuditSchedule(ILogger<PMEAuditSchedule> logger, IDBService dbService, IRepairBetRecordService repairBetRecordService, ISystemParameterDbService systemParameterDbService)
        {
            _logger = logger;
        ...
        public async Task Invoke()
        {
            ...
            _logger.LogInformation("Invoke PMEAuditSchedule on time : {time}", DateTime.Now.ToLocalTime());
            try
            {
                ...
                if (parameter == null)
                {
                    var model = new t_system_parameter()
                    {
                        ...
                        name = "PME 每小時遊戲帳務比對排程",
                        description = "PME 紀錄帳務比對排程時間基準點"
                    };
                ...
                else
                {
                    ...
                    {
                        _logger.LogInformation("PME audit schedule stop time: {time}", parameter.value);
                    ...
                    {
                        _logger.LogInformation("return PME audit schedule current Time : {now} report time : {reportTime} ", now, parameter.value);
                        return; // 時間不變就結束排程
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(ex, "Run PME audit schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", ex.GetType().FullName, ex.Message, errorFile, errorLine);
            }
        }
    }
}
