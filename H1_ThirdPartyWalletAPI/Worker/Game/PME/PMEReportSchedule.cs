...
public class PMEReportSchedule : IInvocable
{
    private readonly ILogger<PMEReportSchedule> _logger;
    private readonly IPMEInterfaceService _apiInterfaceService;
    private readonly IDBService _dbService;
    private readonly ISystemParameterDbService _systemParameterDbService;

    public const string SYSTEM_PARAMETERS_KEY = "PMEReportSchedule";
    public PMEReportSchedule(ILogger<PMEReportSchedule> logger, IPMEInterfaceService apiInterfaceService, IDBService dbService, ISystemParameterDbService systemParameterDbService)
    {
        _logger = logger;
    ...
    public async Task Invoke()
    {
        ...
        _logger.LogInformation("{platform} Invoke PMEReportSchedule on time : {time}", Platform.PME, DateTime.Now);
        try
        {
            ...
            if (parameter == null)
            {
                var model = new t_system_parameter()
                {
                    ...
                    name = "PME 每小時遊戲匯總報表排程",
                    description = "PME 紀錄排程時間基準點"
                };
            ...
            else
            {
                ...
                {
                    _logger.LogInformation("{platform} report schedule stop time: {time}", Platform.PME, parameter.value);
                ...
                {
                    _logger.LogInformation("return {platform} report schedule current Time : {now} report time : {reportTime} ", Platform.PME, now, parameter.value);
                    return; // 時間不變就結束排程
                }
            }
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError("Run {platform} report schedule exception EX : {ex}  MSG : {Message}  Error Line : {errorFile}.{errorLine}", Platform.PME, ex.GetType().FullName, ex.Message, errorFile, errorLine);
        }
    }
}
